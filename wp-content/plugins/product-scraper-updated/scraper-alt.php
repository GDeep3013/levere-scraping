...
//         if ($dom) {
//             // Find product links in the grid
//             $links = $dom->find('ul.product-grid_gridList__q6ju2 li.product-grid-tile_tileListItem___80_W a.al-grid__tile');
            
//             if (empty($links)) {
//                 // Fallback: find any link inside the product grid
//                 $links = $dom->find('.product-grid_gridList__q6ju2 a[href]');
//             }
            
//             if (empty($links)) {
//                 // Fallback: find any product tile links
//                 $links = $dom->find('li[class*="tileListItem"] a[href]');
//             }
...
function ps_get_product_urls_from_category($category_url) {
    ...
    if (is_wp_error($response)) {
        error_log("Category fetch failed: " . $response->get_error_message());
        return array();
    }
    ...
    // Extract from __NEXT_DATA__ (THIS IS THE KEY FIX)
    ...
    if (!empty($matches[1])) {
        ...
        if (json_last_error() === JSON_ERROR_NONE) {
            // Navigate to the tiles.results array (from your debug data)
            $tiles = $data['props']['pageProps']['tiles'] ?? array();
            $results = $tiles['results'] ?? array();
            ...
            if (!empty($results)) {
                ...
                error_log("Found " . count($product_urls) . " products from __NEXT_DATA__ tiles.results");
            }
            
            // Alternative path: sometimes products might be elsewhere
            if (empty($product_urls)) {
                $possible_paths = [
                    $data['props']['pageProps']['products'] ?? null,
                    $data['props']['pageProps']['items'] ?? null,
                    $data['props']['pageProps']['productList'] ?? null,
                    $data['props']['pageProps']['wagtail']['products'] ?? null,
                ];
                
                foreach ($possible_paths as $products) {
                    if (is_array($products) && !empty($products)) {
                        foreach ($products as $product) {
                            $url = $product['url'] ?? $product['link'] ?? $product['href'] ?? null;
                            if ($url) {
                                $product_urls[] = $url;
                            }
                        }
    ...
    // Fallback: HTML parsing if Next.js extraction failed
    if (empty($product_urls) && function_exists('str_get_html')) {
        $dom = str_get_html($html);
        
        if ($dom) {
            // Look for product URLs in the grid
            $all_links = $dom->find('a[href]');
            
            foreach ($all_links as $link) {
                $href = $link->href;
                
                if (!empty($href) && 
                    strpos($href, '/all-products/') !== false && 
                    strpos($href, '/electronic-front-door-entry-sets/') !== false &&
                    substr($href, -1) !== '/') {
    ...
    error_log("Total product URLs found: " . count($product_urls));
