...
class Viefe_Product_Scraper {
    ...
    public function register_api_routes() {
        ...
        register_rest_route(
            ...
            array(
                'methods'             => 'GET',
                'callback'            => array($this, 'get_product_detail_callback'),
                'permission_callback' => '__return_true',
            )
        );
    ...
    public function get_product_detail_callback($request) {
        $url = $request->get_param('url');

        if (empty($url)) {
            return new WP_Error('missing_url', 'Product URL is required', array('status' => 400));
        }

        $product_detail = $this->scrape_product_detail($url);

        if ($product_detail && !empty($product_detail['title'])) {
            return rest_ensure_response($product_detail);
        }

        return new WP_Error('no_data', 'Failed to fetch product details', array('status' => 404));
    }
    ...
    public function get_products_callback($request) {
        ...
        $product_items = $html->find(".module-products .grid-items .product-list form.buyForm");
    ...
    public function get_products_callback___($request) {
        $url = $request->get_param('url');
        ...
        preg_match_all('/https?:[^\s"\'<>]+\.json[^\s"\'<>]*/', $html_content, $api_matches);
        preg_match_all('/\/api\/[^\s"\'<>]+/', $html_content, $api_matches2);
        preg_match_all('/fetch\([^\s"\'<>]+\)/', $html_content, $fetch_matches);
        
        // Look for product data in JSON-LD or script tags
        preg_match('/<script type="application\/ld\+json">(.*?)<\/script>/s', $html_content, $json_ld);
        
        return rest_ensure_response(array(
            'possible_apis' => array_merge($api_matches[0], $api_matches2[0]),
            'fetch_calls' => $fetch_matches[0],
            'has_json_ld' => !empty($json_ld[1]),
            'html_length' => strlen($html_content),
            'has_buyform' => strpos($html_content, 'buyForm') !== false,
            'has_data_product' => strpos($html_content, 'data-product') !== false,
        ));
    }
    ...
    public function extract_product_data_basic($item, $domain, $category_name) {
        ...
        $image_outer = $item->find('.product-list-img-link', 0);
        if ($image_outer) {
            $image_element = $item->find('.product-list-img-size img', 0);
    ...
    public function scrape_product_detail($url) {
        ...
        // if ($drawing_img) {
        //     $product_data['drawing'] = $drawing_img->getAttribute('src') ?? '';
        //     echo 'Drawing found: ' . $product_data['drawing'];
        // } else {
        //     echo 'No drawing found for URL: ' . $url;
        // }


        $drawing_img = $detail_html->find('.tag-wrapper.drawing img, #collapse-drawing img', 0);
        if (!$drawing_img) {
            foreach ($detail_html->find('img') as $img) {
                $src = $img->getAttribute('src') ?? '';
                if (stripos($src, '_D1') !== false || stripos($src, '_D1_') !== false) {
                    $drawing_img = $img;
                    break;
                }
            }
        ...
        $product_data['drawing'] = $drawing_img ? ($drawing_img->getAttribute('src') ?? '') : '';
...
function viefe_ajax_init_scrape() {
    ...
    if (!$product_items || count($product_items) === 0) {
        $html->clear();
        unset($html);
        wp_send_json_error('No products found on this pagess.');
    }
    ...
    if (!$skip_details && !empty($product['url'])) {
        ...
        if ($product_detail) {
            ...
            if (!empty($product_detail['drawing'])) {
                $product['drawing'] = $product_detail['drawing'];  // ← UNCOMMENT THIS LINE
                $product['drawing_image'] = $product_detail['drawing'];  // Keep this too
            }
